"""Convenience script for fine-tuning models."""
...
@hydra.main(
    ...
    config_name="finetune/full_base",
    version_base=None,
)
def main(cfg: DictConfig):
    """
    Fine-tune a model on the full TOFU subset for both training and validation.

    This is a convenience wrapper around the main training script
    that uses the fine_tune configuration by default.

    Usage:
        python script/train/finetune_full.py
        python script/train/finetune_full.py model.name=meta-llama/Llama-3.2-3B
        # Local JSON/JSONL override (bypasses HF dataset)
        python script/train/finetune_full.py data.train_file=data/raquel/affected_train.json data.val_file=data/raquel/affected_val.json
    """
    train_main(cfg)
