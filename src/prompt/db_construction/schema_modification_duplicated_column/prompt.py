...
class SchemaColumnDeduplicationSignature(dspy.Signature):
    ...
    Output_Schema: str = dspy.OutputField(
        description="Output schema of an Aligned Database in SQL CREATE TABLE format with semantically equivalent columns merged"
    )


@dataclass
class SchemaColumnDeduplicationPrompt(Prompt):
    schema: str
    ...
    def signature(cls) -> Type[dspy.Signature]:
        return SchemaColumnDeduplicationSignature
