...
class UpdateNullifyGenerationSignature(dspy.Signature):
    ...
    Update_Statement: str = dspy.OutputField(
        description="A PostgreSQL-compatible UPDATE statement that nullifies the content"
    )


@dataclass
class UpdateNullifyGenerationPrompt(Prompt):
    schema: List[str]
    ...
    def signature(cls) -> Type[dspy.Signature]:
        return UpdateNullifyGenerationSignature
